...
def register_schedule_routes(app, mysql, login_required, roles_required):
    ...
    @app.route('/api/schedule/<int:schedule_id>/event-on-date', methods=['GET'])
    @login_required
    def get_schedule_event_on_date(schedule_id):
        """ 
        Get the EventID generated by a schedule for a specific date.
        """
        ...
        try:
            ...
            try:
                ...
                if result:
                    return jsonify({'success': True, 'event_id': result['EventID']}), 200
                return jsonify({'success': False, 'message': 'No event found for this schedule'}), 404
            finally:
                cursor.close()
        except Exception as e:
            print(f"Error getting event for schedule date: {str(e)}")
